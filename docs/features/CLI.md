# CLI Commands: Developer Analytics

The claudewatch CLI gives you structured visibility into your Claude Code sessions—the human-facing side of the dual observability layer. It reads local data under `~/.claude/`, computes metrics from session patterns, and stores snapshots in a local SQLite database. No network calls, no telemetry, everything stays on your machine.

These commands are run outside sessions for analysis, improvement, and tracking. For in-session real-time data—friction patterns, cost, and agent performance surfaced to Claude while decisions are being made—see [MCP Tools](/docs/features/MCP_TOOLS.md).
...
1. **Core workflow** - scan, metrics, gaps, suggest, fix, track
2. **Memory management** - memory status, show, extract, clear
3. **Monitoring** - watch, hook, startup
4. **Analysis** - search, compare, anomalies, correlate, attribute, replay
5. **Experimentation** - experiment start, tag, report, stop
6. **System** - install, doctor
...
**Use case:** Baseline measurement before making CLAUDE.md improvements. Run before and after to validate whether changes increased scores.
...
**Use case:** Comprehensive retrospective analysis. Run weekly or monthly to identify trends, validate improvements, or diagnose cost increases.
...
**Use case:** Quick diagnostic to identify what's missing before running `suggest` or `fix`. Focuses on structural issues rather than trends.
...
**Use case:** Data-driven prioritization. Instead of guessing what to improve, this ranks opportunities by projected impact: `(affected_sessions × frequency × time_saved) / effort`.
...
**Use case:** Apply data-driven improvements automatically. Rule-based mode analyzes your patterns and adds targeted CLAUDE.md sections. AI mode generates custom content from project structure and history.
...
**Use case:** Before/after measurement. Snapshot before making CLAUDE.md changes, work for a week, then run `track --compare` to validate whether friction decreased and cost-per-commit improved.
...
**Use case:** Quick overview of cross-session task history and blocker tracking across all projects.
...
**Use case:** Review what was attempted before, what blockers were encountered, and what solutions were applied. Queryable via MCP tools (`get_task_history`, `get_blockers`) during sessions.
...
**Use case:** Checkpoint progress mid-session before risky operations, or after completing major work to preserve context for future sessions.
...
**Use case:** Clear stale task history when starting fresh on a project, or remove memory for projects no longer actively developed.
...
**Use case:** Passive monitoring for long-running work. Get notified when friction spikes or patterns change without manually running `metrics`.
...
**Use case:** Automatic behavioral intervention during sessions. Fires when thresholds are crossed, telling Claude what to do without requiring manual checks.
...
**Use case:** Automatic project health briefing at session start. Surfaces friction patterns, agent success rates, and regression status before approach decisions are made.
...
**Use case:** Find sessions where a specific topic, error, or tool was discussed before tackling a recurring problem. Queryable via `search_transcripts` MCP tool during sessions.
...
**Use case:** Validate whether Scout-and-Wave parallelization is effective for your workflow. Compare cost-per-commit and friction between SAW and sequential sessions.
...
**Use case:** Identify unusually expensive or high-friction sessions. Review what went wrong in outlier sessions to prevent recurrence.
...
**Use case:** Data-driven analysis of what affects outcomes. Discover which session attributes correlate with low friction, high commit rates, or efficient cost-per-outcome.
...
**Use case:** Post-mortem analysis of expensive sessions. Identify whether Agent, Bash, Read, or other tool types drove cost.
...
**Use case:** Turn-by-turn review of what happened during a session. Identify where cost spiked, where errors occurred, and what the conversation flow looked like.
...
**Use case:** A/B test CLAUDE.md changes. Create two variants, tag sessions to each, then compare outcomes to determine which approach works better.
...
**Use case:** One-time setup to enable hooks and behavioral rules. Re-run after upgrading claudewatch to update hook configuration for new features.
...
**Use case:** Diagnostic tool for troubleshooting configuration issues. Run before opening a bug report to identify misconfiguration.
...
- [MCP Tools Reference](/docs/features/MCP_TOOLS.md) - In-session self-reflection API
...
- [First Session Walkthrough](/docs/guides/QUICKSTART.md) - From scan to fix
