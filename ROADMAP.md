...
## Phase 3.6: Session-End Memory (v0.16.0) ✅ COMPLETE

**Goal:** Automatic memory extraction when sessions close

### 3.6.1 Stop hook - automated memory extraction ✅ COMPLETE
**Impact:** 🔥🔥🔥 High - Completes memory lifecycle (SessionStart load → PostToolUse monitor → Stop checkpoint)
**Effort:** 🛠 Low - Hook implementation complete
**Dependencies:** `extract_current_session_memory` (exists), facets (opt-in via `/insights`)
**Status:** ✅ **Shipped - working as designed with graceful degradation**

**How it works:**
- Stop hook implemented in `internal/app/hook_stop.go` with full test suite (27 tests)
...
- Extracts from session-meta (commits, errors, tool counts, duration)
- Optionally uses facets when available (generated by `/insights` command)

**IMPORTANT DISCOVERY:** Facets are NOT automatic
- Facets (AI session analysis) require running `/insights` command
- `/insights` analyzes up to 50 recent sessions and caches results to `~/.claude/usage-data/facets/`
- Uses Claude Haiku to extract: `underlying_goal`, `outcome`, `friction_detail`, `brief_summary`
- **Facets are OPT-IN, not automatic** - this is by design, not a bug
- Reference: https://www.zolkos.com/2026/02/04/deep-dive-how-claude-codes-insights-command-works.html

**Two-tier extraction model:**
1. **Default (metadata-only):** Captures commits, errors, tool counts, duration
   - Automatic on every significant session close
   - Zero user action required
   - Sufficient for basic task tracking
   
2. **Rich (with facets):** Adds AI-analyzed goals, outcomes, solutions, friction
   - Requires running `/insights` command periodically
   - Generates facets for recent sessions
   - Stop hook automatically uses facets when available
...
- ✓ Metadata extraction (commits, errors, duration, tool counts)
- ✓ Graceful degradation (works with or without facets)
- ✓ Facet enrichment (automatically used when `/insights` has run)

**Usage patterns:**
- **For automatic tracking:** Do nothing - Stop hook captures metadata automatically
- **For rich analysis:** Run `/insights` periodically (weekly/monthly) to generate facets for recent sessions
- **For immediate rich extraction:** Run `/insights` after important sessions, then let Stop hook use the facets

**Next steps (future enhancements):**
1. **Optional:** Add suggestion in PostToolUse hook: "Report /insights bug to Anthropic or wait for rich memory extraction"
2. **Optional:** Enhance metadata-only extraction to infer more from patterns (commits, tool usage, errors)

**Success metric:** ✅ Memory extraction happens automatically for 100% of significant sessions (metadata captured)
**Bonus metric:** Users who run `/insights` get AI-analyzed goals + outcomes

**Priority:** ⭐⭐⭐ SHIPPED

**Deliverable:** v0.16.0 - "Session-End Memory (Complete)"
...
### v0.17.0 - "Collective Intelligence" (Target: 8-12 weeks after v0.15.0)
**Deliverables:**
...
### v0.16.0 - "Session-End Memory (Complete)" (Target: 1 week after v0.15.5) ✅ SHIPPED
**Deliverables:**
...
**Known limitations:**
- **`/insights` command broken as of 2026-03-06** (Claude Code internal issue)
  - Error: `TypeError: Object.entries requires that input parameter not be null or undefined`
  - This is the command that generates facets (AI-analyzed session goals/outcomes)
  - Stop hook is working correctly but can't access facets until `/insights` is fixed
  - Bug is upstream in Claude Code, not claudewatch

**Workaround considered but deferred:**
- Could implement `claudewatch insights` using prompts from https://www.zolkos.com/2026/02/04/deep-dive-how-claude-codes-insights-command-works.html
- Would reuse existing infrastructure: `fixer/` (Claude API client), `claude/transcripts.go` (parsing)
- **Deferred because:** Maintenance burden (keep prompts in sync, remove when fixed) not worth it for nice-to-have feature
- **Decision:** Wait for upstream fix. Metadata-only extraction is sufficient for basic task tracking
- **Reconsider if:** `/insights` remains broken for 1+ month (indicates abandonment, not just a bug)
...
**v0.16.0 Stop Hook: ✅ SHIPPED - WORKING AS DESIGNED**
- ✅ Implementation complete - Stop hook fires on significant sessions
- ✅ Background extraction working - metadata captured automatically
- ✅ Graceful degradation - works with or without facets
- 💡 **DISCOVERY**: Facets are opt-in via `/insights` command, not automatic
  - Facets provide: task goals, outcomes, solutions, friction analysis
  - Generated by running `/insights` (analyzes up to 50 recent sessions)
  - Stop hook automatically enriches extraction when facets exist
  - Reference: https://www.zolkos.com/2026/02/04/deep-dive-how-claude-codes-insights-command-works.html
- ⚠️ **BLOCKER**: `/insights` command currently broken (Claude Code bug)
  - Error: `TypeError: Object.entries requires that input parameter not be null or undefined`
  - Prevents facet generation until fixed upstream
  - Stop hook operates in metadata-only mode until resolved

**Two-tier model:**
1. **Automatic (metadata):** Commits, errors, tool counts - zero user action ✅ WORKING
2. **Rich (with facets):** AI-analyzed goals + outcomes - requires `/insights` ⚠️ BLOCKED (awaiting upstream fix)
