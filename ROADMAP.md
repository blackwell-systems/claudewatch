...
1. **Optional:** Add `claudewatch insights` wrapper that calls `/insights` then extracts from newly generated facets
2. **Optional:** Add suggestion in PostToolUse hook: "Run /insights to enable rich memory extraction"
3. **Optional:** Enhance metadata-only extraction to infer more from patterns (commits, tool usage, errors)
...
**Two-tier model:**
1. **Automatic (metadata):** Commits, errors, tool counts - zero user action
2. **Rich (with facets):** AI-analyzed goals + outcomes - run `/insights` periodically
