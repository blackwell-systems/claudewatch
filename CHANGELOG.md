...
- **Contextual memory surfacing at session start (v0.13.0 Wave 2)** — Automatically extracts keywords from the user's first message, queries task history for matching prior attempts, and displays up to 3 matched tasks with status, blockers, and solutions. Uses simple substring matching with stop word filtering (85+ words), minimum 3-character keywords, case-insensitive. New files: `internal/memory/surface.go`, `internal/memory/surface_test.go`.
- **Periodic session efficiency dashboard (v0.13.0 Wave 2)** — PostToolUse hook now displays a session efficiency dashboard every 50 tool calls showing cost, commits, cost/commit, tool errors, duration, drift percentage, and status emoji (🟢 efficient / 🟡 adequate / 🔴 struggling). Thresholds: efficient requires cost/commit < $1, errors < 10, drift < 20%; struggling triggers on cost/commit > $2 OR errors > 20 OR drift > 30%. New files: `internal/analyzer/dashboard.go`, `internal/analyzer/dashboard_test.go`.

### Changed

- **Per-model cost accuracy for live sessions** — Live session cost calculations (PostToolUse dashboard, cost velocity hook, `get_cost_velocity` MCP tool, `get_session_dashboard` MCP tool) now price each assistant turn at its actual model tier (opus/sonnet/haiku) instead of assuming everything is Sonnet. A session using 50% Opus / 50% Sonnet was previously undercosted by ~2-3x. Added `ModelPricingMap` and `PricingForModel` to `claude/active_live_cost.go`; updated `ComputeSessionDashboard` to use `SessionMeta.ModelUsage` for per-model breakdown; removed hardcoded Sonnet pricing constants from `hook.go`, `cost_velocity_tools.go`, and `dashboard_tools.go`. Sonnet pricing used as fallback only when model field is absent from a turn.

- **Per-model cost accuracy for historical sessions** — `EstimateSessionCost` now uses per-model pricing from `SessionMeta.ModelUsage` when populated, pricing each model's tokens at the correct tier rate (opus $15/$75, sonnet $3/$15, haiku $0.25/$1.25 per million input/output). Falls back to single-tier Sonnet pricing for older sessions without `ModelUsage`. All 20+ call sites across `mcp/`, `app/`, `export/`, and `watcher/` packages benefit automatically — the function signature is unchanged, so `pricing` and `ratio` params serve as fallback only. Added `estimateFromModelUsage` helper using `ClassifyModelTier` and `getPricingForTier`. 19 new tests across analyzer, MCP, app, and export packages verify per-model pricing, mixed-model weighted sums, and fallback behavior.

- **Migrated behavioral instructions from CLAUDE.md to modular rules files** — `claudewatch install` now writes 3 focused rule files to `~/.claude/rules/` instead of injecting a monolithic block into `~/.claude/CLAUDE.md`. Files: `claudewatch-session-start.md` (startup protocol), `claudewatch-session-protocol.md` (during-session behaviors with structured sections), `claudewatch-tools.md` (MCP tool reference). Rules files have the same priority as CLAUDE.md but split by concern so each gets focused attention instead of competing in a single block. Install automatically removes the legacy `<!-- claudewatch:start/end -->` block from CLAUDE.md on upgrade. Fully idempotent — re-running overwrites files in place.

### Fixed

- **Cache token cost accounting (critical)** — All cost calculations were missing `cache_read_input_tokens` and `cache_creation_input_tokens` from the JSONL transcript, causing 10x cost undercounting for long/resumed sessions. A session costing $547 (per `/cost`) was reported as $50. Fixed in 9 files across `claude/`, `analyzer/`, `store/`, `app/`, `mcp/` packages: added cache fields to `assistantMsgUsage`, `assistantMsgWithContent`, `attributionAssistantMsg`, `SessionMeta`, `ModelStats`, `TurnAttribution`, `toolStats` structs; added `CacheReadPerMillion`/`CacheWritePerMillion` to `CostPricing`; updated 6 cost formulas in `ComputeAttribution`, `ParseLiveCostVelocity`, `ComputeSessionDashboard`, per-model breakdown, and hook pricing constants. Cache reads (10% of input rate) and cache writes (125% of input rate) are now included in all cost paths.
...
- **`replay` command now accepts optional session ID** — Previously required `<session-id>` argument, now accepts `[session-id]` (optional). When omitted, uses interactive session selection (same logic as `attribute`). Usage: `claudewatch replay` (select from active), `claudewatch replay abc123` (explicit), `claudewatch replay --from 10 --to 20` (with range).
- **`memory extract` improved multi-session handling** — Replaced old `FindActiveSessionPath` with new `SelectSession(..., RequireActiveSession())`. Now properly handles multiple active sessions with interactive menu (TTY) or error with session list (non-TTY). Previously only supported single active session.
- **`experiment tag` prioritizes active sessions** — Changed from "most recent from all sessions ever" to "active sessions first, then fallback to most recent". More intuitive when tagging sessions during active development. Uses same interactive selection as other commands.
